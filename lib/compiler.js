  ...
  phasesNames.forEach(function(phase) {
    ...
    Object.keys(phaseConfig).forEach(function(middleware) {
      ...
      allConfigs.forEach(function(config) {
        var sourceFile = resolveMiddlewarePath(rootDir, middleware);
...
function resolveMiddlewarePath(rootDir, middleware) {
  ...
  if (start === './' || start === '..')
    return require.resolve(path.resolve(rootDir, middleware));

  var segments = middleware.split('/');
  if (segments.length !== 2)
    return require.resolve(middleware);

  var candidates = [
    middleware,
    segments[0] + '/middleware/' + segments[1],
    segments[0] + '/server/middleware/' + segments[1]
  ];

  var err;
  for (var ix in candidates) {
    try {
      return require.resolve(candidates[ix]);
    }
    catch (e) {
